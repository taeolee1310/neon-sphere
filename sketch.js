// Final stable version — Neon sphere + mouse magnet + dark gradient + color themes
...
function draw() {
  ...
  let globalPulse = 1 + 0.05 * Math.sin(t*0.9);
  ...
  for (let p of points) {
    ...
    let wLat =
      0.18 * Math.sin(lon0*3 + waveT*1.2) +
      0.06 * Math.sin(lat0*6 - waveT*1.8);

    let wLon =
      0.16 * Math.sin(lat0*2.4 - waveT*1.0) +
      0.05 * Math.sin(lon0*5 + waveT*2.1);
    ...
    // --- dot size ---
    let dotSize =
      (BASE_SIZE + MAX_EXTRA_SIZE*depth*(0.35 + 0.65*shade)) *
      (1 + 0.25*(waveB-0.5)) *
    ...
    // --- color ---
    let col = computeColor(currentTheme, rim, shade, waveB, p.bulge, depth, globalPulse);
...
//
// NEON theme color calculation
//
function computeColor(theme, rim, shade, waveB, bulge, depth, pulse) {
